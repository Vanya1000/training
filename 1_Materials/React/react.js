...
+ useLayoutEffect
  ...
  При useEffect на основе return создается виртуальное дерево => далее оно отдается на отрисовку в браузер и только после этого вызывается функция переданная в useEffect 
------------
...
+MobX
  Redux
    1. Один стор - одна точка входа
    2. Иммутабельность
    3. Стейт меняется только через экшены
    4. Стейт меняется в редьюсерах
  MobX
    1. Несколько сторов
    2. Гибкость
    3. Все подписчики на стейт обновляются
