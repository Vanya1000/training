  ...
  +Global object window
    ...
    ?History API (Global object window)
      ...
      Основные методы объекта History:
        ...
        -window.history.forward(): Метод, идентF#ичный вызову go(1)
    ...
    ?Parsing
      ...
      Layout / Reflow Правильнее будет сказать Layout для первой операции, а все дальнейшие повторные операции, будет правильнее называть Reflow. Иногда этот процесс называется просто Layout, во всех случаях.
        4. Вычисление позиции и размеров, Layout Для каждого элемента render tree рассчитывается положение на странице — происходит layout. Браузеры используют поточный метод (flow), при котором в большинстве случаев достаточно одного прохода для размещения всех элементов (для таблиц проходов 
          ...
          Глобальный и инкрементальный Layout Секция статьи "Глобальный и инкрементальный Layout"
...
!Design patterns:
  ...
  +Intermediate knowledge of patterns and best practices:
    ...
    Некоторые проблемы слишком малы для выделения в архитектурный подход, но достаточно часто встречаются, чтобы породить стандартные решения. Такие стандартные решения называются паттернами или шаблонами.
      Порождающие - Отвечают за удобное и безопасное создание новых объектов или даже целых семейств объектов. например, фабричный метод, абстрактная фабрика, строитель, прототип, одиночка.
      Структурные - Отвечают за построение удобных в поддержке иерархий классов. например, адаптер, мост, компоновщик, декоратор, фасад, легковес, заместитель.
      Поведенческие - Решают задачи эффективного и безопасного взаимодействия между объектами программы.  например, цепочка обязанностей, команда, итератор, посредник, снимок, посетитель, стратегия.
    ?KISS, DRY, YAGNI
    1. YAGNI
...
!Web Communication Protocols: (optional)
  ...
  +HTTP vs HTTPS vs HTTP/2
    ...
    HTTPS (аббр. от англ. HyperText Transfer Protocol Secure) — расширение протокола HTTP для поддержки шифрования в целях повышения безопасности. 
      Данные в протоколе HTTPS передаются поверх криптографических протоколов TLS или устаревшего в 2015 году SSL[1]. В отличие от HTTP с TCP-портом 80, для HTTPS по умолчанию используется TCP-порт 443[2].
