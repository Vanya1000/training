...
?Список (связный список):
  ...
  У каждого узла есть value (значение этого элемента(узла)) и nextNode (ссылка на следующий элемент связного списка).
    ...
    Плох для индексирования и поиска (сложно получить элемент по его номеру или просто найти элемент из-за вложенности).
      ...
      const node = new List("initial");
      node.next = new List("1");
?Стек
    ...
    *-Полиморфизм
    ...
    Выделяют 2 основных типа полиморфизма. 
      ...
      *Параметрический(истинный)
      Полиморфизм - это концепция в объектно-ориентированном программировании, которая позволяет объектам разных классов иметь общий интерфейс или методы с одним именем, но с разными реализациями. 
      Это означает, что разные объекты могут откликаться на одну и ту же команду или метод по-разному, в зависимости от их типа или класса. Полиморфизм позволяет писать более гибкий и модульный код, 
      так как он способствует повторному использованию и упрощению кода.
